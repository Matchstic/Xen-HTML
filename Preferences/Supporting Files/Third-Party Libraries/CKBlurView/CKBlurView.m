...
@interface _MCBlurView : UIView
@end

@interface MCBlurView () {
    _MCBlurView *_blurView;
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        _blurView = [[_MCBlurView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)];
        [self addSubview:_blurView];
        
        CAFilter *filter = [CAFilter filterWithName:kCAFilterGaussianBlur];
        _blurView.layer.filters = @[ filter ];
